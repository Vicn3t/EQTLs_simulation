...
for (i in 1:m) {
  ...
  return(list(scores=scores,betas=betas))
} #end score.calc
...
if ((n.core > 1) & requireNamespace("parallel",quietly=TRUE)) {
    	...
    	scores_full <- unlist(parallel::mclapply(it,function(markers){score.calc(M[ix.pheno,markers])},mc.cores=n.core))
    	scores <- scores_full[[1]]
	 } else {
    	scores_full <- score.calc(M[ix.pheno,])
    	scores <- scores_full[[1]]
	 }
	 if (plot) {
	 	qq(scores)
...
return(data.frame(map,scores_full,all.scores))
